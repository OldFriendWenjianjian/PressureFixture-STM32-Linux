...
const u32 POINT_COLOR_TBL[5]={color16M_red,color16M_green,color16M_blue,color16M_black,color16M_gray}; 
...
void Load_Drow_Dialog(void)
{
	...
	LT768_Print_Internal_Font_String(1024-40,0,color16M_red,color16M_white,"RST");
...
//size：线条的粗细程度
//color：线条的颜色
void lcd_draw_bline(u16 x1, u16 y1, u16 x2, u16 y2,u8 size,u32 color)
{
	u16 t; 
		...
		for(t=0;t<5;t++)  //电容屏支持的点数,固定为5点
		{
			if((tp_dev.sta)&(1<<t))
			{
				if(tp_dev.x[t]<1024&&tp_dev.x[t]>0&&tp_dev.y[t]<600&&tp_dev.y[t]>0)
				{
					...
					lastpos[t][0]=tp_dev.x[t];
					lastpos[t][1]=tp_dev.y[t];
					if(tp_dev.x[t]>(1024-50)&&tp_dev.y[t]<30)
					{
						Load_Drow_Dialog();//清除
					}
				}
...
{ 
	...
	StartUp_picture();
	//LT768_PWM1_Init(1,0,200,100,50);
	//LT768_PWM1_Init(1,0,20,100,50);//PWM频率=50KHz
	LT768_PWM1_Init(1,0,50,100,100);//PWM频率=20KHz
	//WaitKey();
    //Load_Drow_Dialog();
	//ctp_test();
	...
	*/
	...
	while(1)
	{
		...
		//LT768_DrawCircle_Fill(512,300,100,color16M_white);
		//WaitKey();
		//Color_Bar_ON();
		//WaitKey();
		//Color_Bar_ON();
