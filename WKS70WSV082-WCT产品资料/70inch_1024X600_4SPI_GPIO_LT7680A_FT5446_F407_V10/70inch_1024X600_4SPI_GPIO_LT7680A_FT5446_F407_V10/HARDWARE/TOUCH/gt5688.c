...
u8 GT5688_Init(void)
{
	...
	printf("CTP ID:%s\r\n",temp);	//打印ID
	//GT5688_Send_Cfg(1);
	if(strcmp((char*)temp,"5688")==0)//ID==9147
	{
		temp[0]=0X02;			
		GT5688_WR_Reg(GT_CTRL_REG,temp,1);//软复位GT5668
		...
		GT5688_RD_Reg( GT_CFGS_REG, Cfg_Info, sizeof(Cfg_Info) );	
		printf("Config Info:\r\n");
		for( i = 0; i < sizeof(Cfg_Info); i++ )
		{
			if(Cfg_Info[i]<16)
			printf("0x0%X,",Cfg_Info[i]);
			else printf("0x%X,",Cfg_Info[i]);
			if(((i+1)%16)==0)
			printf("\r\n");
		}
		printf("\r\n");
		...
		if((mode&0XF)&&((mode&0XF)<6))
		{
			...
			for(i=0;i<5;i++)
			{
				if(tp_dev.sta&(1<<i))	//触摸有效?
				{
					...
					if(tp_dev.x[i]>0&&tp_dev.x[i]<800&&tp_dev.y[i]>0&&tp_dev.y[i]<480)
